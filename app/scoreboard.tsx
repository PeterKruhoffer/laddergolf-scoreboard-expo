    ...
    useEffect(() => {
        if (leftScore >= 21 || rightScore >= 21) {
            if (leftScore === 21) {
                ...
                createAlert(player1Name, router, setPlayer1Name, setPlayer2Name, game)
            }
            if (rightScore === 21) {
                const game: Game = {
                    player1Name: player1Name,
                ...
                createAlert(player2Name, router, setPlayer2Name, setPlayer1Name, game)
            }
        }
        ...
        <>
            ...
            <View className="bg-white h-full flex-1 flex-row justify-center items-center py-3">
                <View className="w-1/2 border-r-2 releative border-slate-200 h-full flex-1 justify-evenly items-center">
                    <Text className="text-2xl top-0 absolute underline">{player1Name}</Text>
                ...
                <View className="w-1/2 border-l-2 relative border-slate-200 h-full flex-1 justify-evenly items-center">
                    <Text className="text-2xl top-0 absolute underline">{player2Name}</Text>
...
function createAlert(title: string, router: any, resetName1: any, resetName2: any, game: Game) {
